...
[[nodiscard]] auto to_parameter_value(rsl::StaticVector<T, capacity> const& value) {
  ...
  public:
    ...
    void declare_params(){
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "action_name") << ": " << param.get_type_name() << " = " << param.value_to_string());
      updated_params.action_name = param.as_string();
      param = parameters_interface_->get_parameter(prefix_ + "tick_frequency");
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "tick_frequency") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = bounds<int64_t>(param, 1, 1000);
        !validation_result) {
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "groot2_port") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = bounds<int64_t>(param, 1, 49151);
        !validation_result) {
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "plugins") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = unique<std::string>(param);
        !validation_result) {
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "ros_plugins_timeout") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = bounds<int64_t>(param, 1, 10000);
        !validation_result) {
      ...
      RCLCPP_DEBUG_STREAM(logger_, (prefix_ + "behavior_trees") << ": " << param.get_type_name() << " = " << param.value_to_string());
      if(auto validation_result = unique<std::string>(param);
        !validation_result) {

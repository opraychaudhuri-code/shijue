// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from rmoss_interfaces:srv/ExchangeAmmon.idl
// generated code does not contain a copyright notice
#include "rmoss_interfaces/srv/detail/exchange_ammon__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

// Include directives for member types
// Member `robot_name`
#include "rosidl_runtime_c/string_functions.h"

bool
rmoss_interfaces__srv__ExchangeAmmon_Request__init(rmoss_interfaces__srv__ExchangeAmmon_Request * msg)
{
  if (!msg) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__init(&msg->robot_name)) {
    rmoss_interfaces__srv__ExchangeAmmon_Request__fini(msg);
    return false;
  }
  // ammo_amount
  return true;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Request__fini(rmoss_interfaces__srv__ExchangeAmmon_Request * msg)
{
  if (!msg) {
    return;
  }
  // robot_name
  rosidl_runtime_c__String__fini(&msg->robot_name);
  // ammo_amount
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Request__are_equal(const rmoss_interfaces__srv__ExchangeAmmon_Request * lhs, const rmoss_interfaces__srv__ExchangeAmmon_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->robot_name), &(rhs->robot_name)))
  {
    return false;
  }
  // ammo_amount
  if (lhs->ammo_amount != rhs->ammo_amount) {
    return false;
  }
  return true;
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Request__copy(
  const rmoss_interfaces__srv__ExchangeAmmon_Request * input,
  rmoss_interfaces__srv__ExchangeAmmon_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // robot_name
  if (!rosidl_runtime_c__String__copy(
      &(input->robot_name), &(output->robot_name)))
  {
    return false;
  }
  // ammo_amount
  output->ammo_amount = input->ammo_amount;
  return true;
}

rmoss_interfaces__srv__ExchangeAmmon_Request *
rmoss_interfaces__srv__ExchangeAmmon_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmoss_interfaces__srv__ExchangeAmmon_Request * msg = (rmoss_interfaces__srv__ExchangeAmmon_Request *)allocator.allocate(sizeof(rmoss_interfaces__srv__ExchangeAmmon_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rmoss_interfaces__srv__ExchangeAmmon_Request));
  bool success = rmoss_interfaces__srv__ExchangeAmmon_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Request__destroy(rmoss_interfaces__srv__ExchangeAmmon_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rmoss_interfaces__srv__ExchangeAmmon_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__init(rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmoss_interfaces__srv__ExchangeAmmon_Request * data = NULL;

  if (size) {
    data = (rmoss_interfaces__srv__ExchangeAmmon_Request *)allocator.zero_allocate(size, sizeof(rmoss_interfaces__srv__ExchangeAmmon_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rmoss_interfaces__srv__ExchangeAmmon_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rmoss_interfaces__srv__ExchangeAmmon_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__fini(rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rmoss_interfaces__srv__ExchangeAmmon_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence *
rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * array = (rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence *)allocator.allocate(sizeof(rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__destroy(rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__are_equal(const rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * lhs, const rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rmoss_interfaces__srv__ExchangeAmmon_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence__copy(
  const rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * input,
  rmoss_interfaces__srv__ExchangeAmmon_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rmoss_interfaces__srv__ExchangeAmmon_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rmoss_interfaces__srv__ExchangeAmmon_Request * data =
      (rmoss_interfaces__srv__ExchangeAmmon_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rmoss_interfaces__srv__ExchangeAmmon_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rmoss_interfaces__srv__ExchangeAmmon_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rmoss_interfaces__srv__ExchangeAmmon_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
rmoss_interfaces__srv__ExchangeAmmon_Response__init(rmoss_interfaces__srv__ExchangeAmmon_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    rmoss_interfaces__srv__ExchangeAmmon_Response__fini(msg);
    return false;
  }
  return true;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Response__fini(rmoss_interfaces__srv__ExchangeAmmon_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
  // message
  rosidl_runtime_c__String__fini(&msg->message);
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Response__are_equal(const rmoss_interfaces__srv__ExchangeAmmon_Response * lhs, const rmoss_interfaces__srv__ExchangeAmmon_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  return true;
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Response__copy(
  const rmoss_interfaces__srv__ExchangeAmmon_Response * input,
  rmoss_interfaces__srv__ExchangeAmmon_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  return true;
}

rmoss_interfaces__srv__ExchangeAmmon_Response *
rmoss_interfaces__srv__ExchangeAmmon_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmoss_interfaces__srv__ExchangeAmmon_Response * msg = (rmoss_interfaces__srv__ExchangeAmmon_Response *)allocator.allocate(sizeof(rmoss_interfaces__srv__ExchangeAmmon_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(rmoss_interfaces__srv__ExchangeAmmon_Response));
  bool success = rmoss_interfaces__srv__ExchangeAmmon_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Response__destroy(rmoss_interfaces__srv__ExchangeAmmon_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    rmoss_interfaces__srv__ExchangeAmmon_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__init(rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmoss_interfaces__srv__ExchangeAmmon_Response * data = NULL;

  if (size) {
    data = (rmoss_interfaces__srv__ExchangeAmmon_Response *)allocator.zero_allocate(size, sizeof(rmoss_interfaces__srv__ExchangeAmmon_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = rmoss_interfaces__srv__ExchangeAmmon_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        rmoss_interfaces__srv__ExchangeAmmon_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__fini(rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      rmoss_interfaces__srv__ExchangeAmmon_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence *
rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * array = (rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence *)allocator.allocate(sizeof(rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__destroy(rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__are_equal(const rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * lhs, const rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rmoss_interfaces__srv__ExchangeAmmon_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence__copy(
  const rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * input,
  rmoss_interfaces__srv__ExchangeAmmon_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rmoss_interfaces__srv__ExchangeAmmon_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rmoss_interfaces__srv__ExchangeAmmon_Response * data =
      (rmoss_interfaces__srv__ExchangeAmmon_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rmoss_interfaces__srv__ExchangeAmmon_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rmoss_interfaces__srv__ExchangeAmmon_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!rmoss_interfaces__srv__ExchangeAmmon_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
